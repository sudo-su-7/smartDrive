...
def logout():
    """
    Logout — accepts both GET and POST.
    Order matters: clear session first so Flask-Login cannot restore
    the user from a partially-cleared session, then call logout_user(),
    then explicitly delete all auth cookies from the response.
    """
    session.clear()
    ...
    # Remove Flask-Login remember-me cookie
    ...
    cookie_domain = current_app.config.get("SERVER_NAME", None)
    ...
    response.delete_cookie(session_cookie, domain=cookie_domain, path=cookie_path)
