...
def export_bookings_pdf():
    """Generate a professional, spiral-bound-ready PDF report."""
    from app.utils.reports import generate_booking_report
    ...
    vehicles_map = {
        str(v["_id"]): v
        for v in db.vehicles.find({"_id": {"$in": [ObjectId(i) for i in vehicle_ids if i]}})
    }
    users_map = {
        str(u["_id"]): u
        for u in db.users.find({"_id": {"$in": [ObjectId(i) for i in user_ids if i]}})
    }

    pdf_buffer = generate_booking_report(
        bookings, vehicles_map, users_map,
        start_label="All Time", end_label="Present",
    )
    filename = f"SmartDrive_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
    return send_file(
        pdf_buffer,
...
def export_bookings_excel():
    """Generate a multi-sheet Excel workbook (Bookings, Revenue Summary, Fleet Performance)."""
    from app.utils.reports import generate_excel_report
    ...
    excel_buf = generate_excel_report(bookings, vehicles_map, users_map)
    if not excel_buf:
        flash("Excel export failed — openpyxl not installed.", "danger")
        return redirect(url_for("admin.reports"))

    filename = f"SmartDrive_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
    return send_file(
        excel_buf,
