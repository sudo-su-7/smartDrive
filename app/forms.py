...
class BookingForm(FlaskForm):
    ...
    # Geolocation — populated via JS, validated server-side
    pickup_address = StringField("Pick-up Address", validators=[Optional(), Length(max=300)])
    pickup_lat = StringField("", validators=[Optional()])
    pickup_lng = StringField("", validators=[Optional()])
    dropoff_address = StringField("Drop-off Address", validators=[Optional(), Length(max=300)])
    dropoff_lat = StringField("", validators=[Optional()])
    dropoff_lng = StringField("", validators=[Optional()])
    ...
    def get_pickup_location(self) -> dict:
        lat = self._safe_float(self.pickup_lat.data)
        lng = self._safe_float(self.pickup_lng.data)
        return {
            "address": self.pickup_address.data or "",
            "lat": lat,
            "lng": lng,
        } if self.pickup_address.data else {}

    def get_dropoff_location(self) -> dict:
        lat = self._safe_float(self.dropoff_lat.data)
        lng = self._safe_float(self.dropoff_lng.data)
        return {
            "address": self.dropoff_address.data or "",
            "lat": lat,
            "lng": lng,
        } if self.dropoff_address.data else {}
...
class MpesaPaymentForm(FlaskForm):
    phone = StringField("M-Pesa Phone Number", validators=[
        DataRequired(),
        Regexp(r"^(\+?254|0)[17]\d{8}$", message="Enter a valid Kenyan mobile number (e.g. 0712345678).")
    ])
    submit = SubmitField("Pay with M-Pesa")


# ─────────────────────────────────────────────────────────────────────────────
# Chat message form (fallback non-WS)
# ─────────────────────────────────────────────────────────────────────────────
