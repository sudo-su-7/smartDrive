...
def init_db(app):
    ...
    with app.app_context():
        try:
            ...
            # ── chat_rooms ────────────────────────────────────────────────────
            db.chat_rooms.create_index([("room_id", ASCENDING)], unique=True)
            db.chat_rooms.create_index([("user_id", ASCENDING)])
            db.chat_rooms.create_index([("updated_at", DESCENDING)])

            # ── chat_messages ─────────────────────────────────────────────────
            db.chat_messages.create_index([("room_id", ASCENDING)])
            db.chat_messages.create_index([("created_at", ASCENDING)])

            # ── bookings: mpesa lookup ─────────────────────────────────────────
            db.bookings.create_index([("mpesa_checkout_request_id", ASCENDING)], sparse=True)

            # ── cms ────────────────────────────────────────────────────────────
            # NOTE: No custom index needed on _id — MongoDB enforces uniqueness
            # on _id automatically for every collection.
            db.cms.create_index([("slug", ASCENDING)], unique=True, sparse=True)

            logger.info("Database indexes ensured.")

            # ── Seed admin ────────────────────────────────────────────────────
            _seed_admin(db, app)
            # BUG FIX: _seed_vehicles was defined but never called
            _seed_vehicles(db)
